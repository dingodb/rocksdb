 ...
 * Class:     org_rocksdb_TtlDB
 * Method:    openCF1
 * Signature: (JLjava/lang/String;[[B[J[IZZ)[J
 */
jlongArray Java_org_rocksdb_TtlDB_openCF1(
    JNIEnv* env, jclass, jlong jopt_handle, jstring jdb_path,
    jobjectArray jcolumn_names, jlongArray jcolumn_options,
    jintArray jttls, jboolean jread_only, jboolean jhas_timestamp_suffix) {
  const char* db_path = env->GetStringUTFChars(jdb_path, nullptr);
  if (db_path == nullptr) {
    // exception thrown: OutOfMemoryError
    return 0;
  ...
  ROCKSDB_NAMESPACE::DBWithTTL* db = nullptr;
  ROCKSDB_NAMESPACE::Status s = ROCKSDB_NAMESPACE::DBWithTTL::Open(
      *opt, db_path, column_families, &handles, &db, ttl_values, jread_only, jhas_timestamp_suffix);
